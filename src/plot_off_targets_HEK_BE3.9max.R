...
# untreated (NT)
# unsorted BE3.9max-treated (US)
# GFP-sorted BE3.9max-treated (GFP)
...
edits_raw = read_tsv('data/rhampseq/HEK/HEK.ALL.allele.edit.tsv', col_types=cols(),show_col_types = FALSE)  %>%
  clean_names() %>%
...
HEK_samples = read_tsv('data/rhampseq/HEK/HEK_samples.tsv', col_types=cols(),show_col_types = FALSE)
HEK_meta = read_tsv('data/rhampseq/HEK/HEK_meta.tsv', col_types=cols(),show_col_types = FALSE)
...
GFP_sorted <- plottable %>%
  filter(treatment == 'GFP-sorted BE3.9max-treated' | treatment == 'untreated')
unsorted <- plottable %>%
  filter(treatment == 'unsorted BE3.9max-treated' | treatment == 'untreated') 
...
GFP_sorted_p <- GFP_sorted %>%
  ...
  summarize(.groups = 'keep',
            ttest_p = t.test(p_edited[treatment=='untreated'],p_edited[treatment=='GFP-sorted BE3.9max-treated'],alternative='less')$p.value) %>%
    mutate(treatment="GFP-sorted BE3.9max-treated")
...
unsorted_p <- unsorted %>%
  ...
  mutate(p_edited = replace_na(p_edited,0)) %>%
  summarize(.groups = 'keep',
            ttest_p = t.test(p_edited[treatment=='untreated'],p_edited[treatment=='unsorted BE3.9max-treated'],alternative='less')$p.value) %>%
    mutate(treatment="unsorted BE3.9max-treated")
...
leg = tibble(
  treatment = c('GFP-sorted BE3.9max-treated','GFP-sorted BE3.9max-treated','unsorted BE3.9max-treated','unsorted BE3.9max-treated'),
...
sorted_plot_reads <- GFP_sorted_p %>% 
  ...
  summarize(.groups = "keep", 
            p_sorted = mean(ttest_p[treatment == "GFP-sorted BE3.9max-treated"]),
            mean_editing_sorted =mean(p_edited[treatment == 'GFP-sorted BE3.9max-treated'], na.rm = TRUE) - mean(p_edited[treatment == "untreated"], na.rm = TRUE),
  ) %>%
  ungroup() %>%
  mutate(mean_editing_sorted=pmax(mean_editing_sorted,0)) %>%
  inner_join(match_amplicon, by= 'amplicon') %>%
  inner_join(read_count, by='grna') %>%
  mutate(read_count =nuclease_read_count, treatment="GFP-sorted BE3.9max-treated") %>%
  select(-grna,-nuclease_read_count) %>%
...
unsorted_plot_reads <- unsorted_p %>% 
  group_by(amplicon,gRNA,strand,significant) %>%
  summarize(.groups = "keep", 
            p_unsorted = mean(ttest_p[treatment == "unsorted BE3.9max-treated"]),
            mean_editing_unsorted = mean(p_edited[treatment == 'unsorted BE3.9max-treated'], na.rm = TRUE) - mean(p_edited[treatment == "untreated"],na.rm = TRUE),
  ) %>%
  ungroup() %>%
  mutate(mean_editing_sorted=pmax(mean_editing_unsorted,0)) %>%
  inner_join(match_amplicon, by= 'amplicon') %>%
  inner_join(read_count, by='grna') %>%
  mutate(read_count =nuclease_read_count, treatment="unsorted BE3.9max-treated") %>%
  select(-grna,-nuclease_read_count) %>%
  merge(leg, by=c("significant", "treatment"))
...
## HEK_BE3.9max_sorted 
resx=300
png('display_items/HEK_BE3.9max_rhampseq_summary_sorted.png',width=4*resx,height=8.5*resx, res=resx)
...
#legend('topright', legend=HEK_meta$treatment, col=HEK_meta$color, text.col=HEK_meta$color, pch=1, bty='n', cex=0.5)
...
resx=300
png('display_items/HEK_BE3.9max_rhampseq_summary_unsorted.png',width=4*resx,height=8.5*resx, res=resx)

plottable<- plottable %>%
  filter(treatment != "GFP-sorted BE3.9max-treated")
...
smry <-plottable %>%
  ...
  summarize(.groups='keep',
            ...
            l95 = lower(p_edited),
            u95 = upper(p_edited)) %>%
  ungroup()  %>% filter(treatment != "GFP-sorted BE3.9max-treated") 
...
resx=300
png('display_items/HEK_BE3.9max_read_count_summary.png',width=6*resx,height=5*resx, res=resx)
...
write_tsv(amplicon_smmary, "output/20231014_HEK_BE39max_indiv.tsv") 


GFP_sorted_reads <- GFP_sorted_p %>% 
  group_by(amplicon,strand) %>%
  summarize(.groups = "keep", 
            p_value = mean(ttest_p[treatment == "GFP-sorted BE3.9max-treated"]),
            editing_GFP_sorted =mean(p_edited[treatment == 'GFP-sorted BE3.9max-treated'], na.rm = TRUE),
...
unsorted_reads <- unsorted_p %>% 
  group_by(amplicon,strand) %>%
  summarize(.groups = "keep", 
            p_value = mean(ttest_p[treatment == "unsorted BE3.9max-treated"]),
            editing_unsorted =mean(p_edited[treatment == 'unsorted BE3.9max-treated'], na.rm = TRUE),
            editing_untreated =mean(p_edited[treatment == "untreated"], na.rm = TRUE),
...
write_tsv(GFP_sorted_reads, "output/20231014_HEK_sorted_BE39max_smry.tsv") 
write_tsv(unsorted_reads, "output/20231014_HEK_unsorted_BE39max_smry.tsv") 
...
#saveWorkbook(wb, "output/20231014_HEK_BE39max_smry.xlsx", overwrite = TRUE)
